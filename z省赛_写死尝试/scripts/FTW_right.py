...
LINE_SEARCH_ROTATION_SPEED_DEG = -7.0 # 丢线后原地向右旋转搜索的速度 (度/秒)
...
STOP_ZONE_ROI_HEIGHT_PX = 6        # 从图像底部向上计算的窗口高度
STOP_ZONE_ROI_WIDTH_PX = 40       # 窗口宽度
        ...
        # 处理需要视觉的状态
            ...
            if self.current_state == FOLLOW_RIGHT:
                ...
                if obstacle_detected:
                    rospy.loginfo("状态转换: FOLLOW_RIGHT_WITH_AVOIDANCE -> AVOIDANCE_MANEUVER")
                    self.stop()
            ...
            if self.current_state == FOLLOW_RIGHT or self.current_state == FOLLOW_RIGHT_WITH_AVOIDANCE or self.current_state == FOLLOW_TO_FINISH:
                # PID巡线逻辑
                if is_line_found:
            ...
            if not is_line_found:
                # 只有在需要巡线的状态下才旋转搜索
                if self.current_state in [FOLLOW_RIGHT, FOLLOW_RIGHT_WITH_AVOIDANCE, FOLLOW_TO_FINISH, STRAIGHT_TRANSITION]:
