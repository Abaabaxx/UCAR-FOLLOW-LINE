...
def create_ipm_video(input_path: str, output_path: str, forward_ipm_matrix: np.ndarray, size: tuple) -> None:
    ...
    while True:
        ...
        # 1. 对【整张】原始图像应用逆透视变换，生成【完整】的俯视图
        ipm_frame = cv2.warpPerspective(frame, forward_ipm_matrix, OUTPUT_SIZE, flags=cv2.INTER_LINEAR)
        
        # 2. 从完整的俯视图中，裁剪出我们感兴趣的ROI区域
        analysis_roi = ipm_frame[IPM_ROI_Y : IPM_ROI_Y + IPM_ROI_H, IPM_ROI_X : IPM_ROI_X + IPM_ROI_W]
        
        # 3. 将【裁剪后的ROI】写入输出视频文件
        out.write(analysis_roi)
...
if __name__ == '__main__':
    ...
    output_filename = f"{base_name}_ipm_roi.mp4" # <--- 修改文件名
    output_path = os.path.join(OUTPUT_DIR, output_filename)
    
    # 定义最终输出视频的尺寸，必须与ROI尺寸相匹配
    output_video_size = (IPM_ROI_W, IPM_ROI_H) # <--- 新增尺寸定义
    
    print(f"开始处理视频: {VIDEO_PATH}")
    print(f"输出将保存到: {output_path}")
    
    # 调用核心功能，注意尺寸参数的变化
    create_ipm_video(VIDEO_PATH, output_path, ipm_matrix, output_video_size)
