...
# 自适应Canny边缘检测参数
ADAPTIVE_CANNY_SIGMA = 0.33  # 控制阈值相对于中位数的浮动范围
# 图像翻转参数
...
def process_video():
    ...
    while True:
        ...
        # 计算自适应Canny阈值
        v = np.median(gray_frame)  # 使用未模糊的灰度图计算中位数
        low_thresh = int(max(0, (1.0 - ADAPTIVE_CANNY_SIGMA) * v))  # 确保阈值不为负
        high_thresh = int(min(255, (1.0 + ADAPTIVE_CANNY_SIGMA) * v))  # 确保阈值不超过255
        
        # 应用自适应Canny边缘检测
        canny_edges = cv2.Canny(blurred_frame, low_thresh, high_thresh)
        
        # 显示原始帧、灰度帧、高斯模糊后的帧和自适应Canny边缘检测结果
        cv2.imshow('Original Frame', frame)
        cv2.imshow('Grayscale Frame', gray_frame)
        cv2.imshow('Gaussian Blurred Frame', blurred_frame)
        cv2.imshow('Adaptive Canny Edges', canny_edges)
