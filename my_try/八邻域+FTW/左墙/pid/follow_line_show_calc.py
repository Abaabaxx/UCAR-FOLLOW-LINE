...
def follow_the_wall(image, start_point):
    """
    使用沿墙走(Follow The Wall)算法跟踪左侧边界
    """
    boundary_points = []
...
class LineFollowerNode:
    ...
    def image_callback(self, data):
        ...
        if left_start_point:
            ...
            # 使用沿墙走算法寻找左边界
            left_points = follow_the_wall(binary_roi_frame, left_start_point)
            ...
            if final_left_border is not None:
                ...
                if roi_points:
                    ...
                    # 找到并绘制胡萝卜点
                    if final_left_border[anchor_y] != -1:
                        carrot_x = final_left_border[anchor_y] + CENTER_LINE_OFFSET
                        if 0 <= carrot_x < roi_w:
