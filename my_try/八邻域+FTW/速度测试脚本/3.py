...
def test_forward_and_rotate(pub, rate):
    """测试同时前进和旋转"""
    print("开始测试同时前进和旋转...")
    print("前进速度: %.2f m/s" % FORWARD_SPEED)
    print("旋转速度: %.1f 度/秒 (%.4f 弧度/秒)" % (ROTATION_SPEED_DEG, ROTATION_SPEED_RAD))
    
    # 创建Twist消息
    twist_msg = Twist()
    twist_msg.linear.x = FORWARD_SPEED      # 设置前进速度
...
if __name__ == '__main__':
    try:
        # 初始化ROS节点
        rospy.init_node('forward_and_rotate_test_node')
        ...
        # 执行前进并旋转测试
        test_forward_and_rotate(pub, rate)
